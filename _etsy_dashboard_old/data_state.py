...
# Auto-calculate Etsy balance from transaction data (matches monolith behavior)
...
etsy_balance = max(0, round(_etsy_all_net - _etsy_deposit_total, 2))
...
# Auto-detect payments from bank transactions (matches monolith)
...
def _reload_etsy_data():
    ...
    # Auto-calculate Etsy balance from transaction data (matches monolith)
    _dep_total = 0.0
...
def _reload_bank_data():
    ...
    # Merge disk-parsed transactions with existing in-memory data
    # (On Railway, only the newly uploaded file exists on disk, so we must
    #  preserve Supabase-loaded rows already in BANK_TXNS.)
    if all_txns and BANK_TXNS:
        existing_keys = set()
        for t in all_txns:
            existing_keys.add((t["date"], f"{float(t['amount']):.2f}", t["type"],
                               t.get("raw_desc", t["desc"])))
        for t in BANK_TXNS:
            key = (t["date"], f"{float(t['amount']):.2f}", t["type"],
                   t.get("raw_desc", t["desc"]))
...
def run_audit():
    """Run 14 sanity checks on all financial data. Returns list of issues."""
    global AUDIT_RESULTS
    issues = []

    # ── 1. Revenue consistency — gross_sales matches sum of Sale rows ────────
    try:
        sale_sum = sales_df["Net_Clean"].sum()
        if abs(sale_sum - gross_sales) > 0.01:
            issues.append({
                "severity": "error",
                "title": "Revenue mismatch",
                "detail": f"gross_sales ({money(gross_sales)}) doesn't match sum of Sale rows ({money(sale_sum)}). Difference: {money(abs(sale_sum - gross_sales))}.",
                "metric": "gross_sales",
            })
    except Exception:
        pass

    # ── 2. Fee ratio — total_fees / gross_sales should be 9–12% ─────────────
    try:
        if gross_sales > 0:
            fee_pct = (total_fees / gross_sales) * 100
            if fee_pct < 9:
                issues.append({
                    "severity": "warning",
                    "title": "Fee ratio unusually low",
                    "detail": f"Fees are {fee_pct:.1f}% of gross sales (expected 9–12%). Could indicate missing fee rows.",
                    "metric": "fee_ratio",
                })
            elif fee_pct > 12:
                issues.append({
                    "severity": "warning",
                    "title": "Fee ratio unusually high",
                    "detail": f"Fees are {fee_pct:.1f}% of gross sales (expected 9–12%). Check for duplicate fee entries.",
                    "metric": "fee_ratio",
                })
    except Exception:
        pass

    # ── 3. Shipping per order — should be $5–$20/order ──────────────────────
    try:
        if order_count > 0 and total_shipping_cost > 0:
            ship_per = total_shipping_cost / order_count
            if ship_per < 5:
                issues.append({
                    "severity": "info",
                    "title": "Low shipping cost per order",
                    "detail": f"Avg shipping is {money(ship_per)}/order (expected $5–$20). Some orders may have free shipping.",
                    "metric": "shipping_per_order",
                })
            elif ship_per > 20:
                issues.append({
                    "severity": "warning",
                    "title": "High shipping cost per order",
                    "detail": f"Avg shipping is {money(ship_per)}/order (expected $5–$20). Check for duplicate shipping labels.",
                    "metric": "shipping_per_order",
                })
    except Exception:
        pass

    # ── 4. COGS vs Revenue — inventory cost shouldn't exceed revenue ────────
    try:
        if gross_sales > 0 and true_inventory_cost > gross_sales:
            issues.append({
                "severity": "warning",
                "title": "COGS exceeds revenue",
                "detail": f"Inventory cost ({money(true_inventory_cost)}) is higher than gross sales ({money(gross_sales)}). Check for personal items in COGS.",
                "metric": "cogs_ratio",
            })
    except Exception:
        pass

    # ── 5. Bank reconciliation — Etsy deposits vs Etsy net payout gap ───────
    try:
        if bank_total_deposits > 0 and abs(etsy_csv_gap) > 50:
            issues.append({
                "severity": "warning" if abs(etsy_csv_gap) < 500 else "error",
                "title": "Bank reconciliation gap",
                "detail": f"Etsy calculated balance vs reported balance gap: {money(etsy_csv_gap)}. Check for missing deposits or statement data.",
                "metric": "bank_reconciliation",
            })
    except Exception:
        pass

    # ── 6. Missing data gaps — months with Etsy but no bank (or vice versa) ─
    try:
        etsy_months = set(months_sorted) if months_sorted else set()
        bank_months_set = set(bank_monthly.keys()) if bank_monthly else set()
        if etsy_months and bank_months_set:
            etsy_only = etsy_months - bank_months_set
            bank_only = bank_months_set - etsy_months
            if etsy_only:
                issues.append({
                    "severity": "warning",
                    "title": "Missing bank statements",
                    "detail": f"Etsy data exists for {', '.join(sorted(etsy_only))} but no matching bank statement found.",
                    "metric": "data_gaps",
                })
            if bank_only:
                issues.append({
                    "severity": "info",
                    "title": "Bank months without Etsy data",
                    "detail": f"Bank statements exist for {', '.join(sorted(bank_only))} but no Etsy CSV covers those months.",
                    "metric": "data_gaps",
                })
    except Exception:
        pass

    # ── 7. Duplicate transactions — same date+amount+description ────────────
    try:
        if len(DATA) > 0:
            dup_cols = ["Date", "Type", "Title", "Net"]
            available = [c for c in dup_cols if c in DATA.columns]
            if len(available) == len(dup_cols):
                dupes = DATA[DATA.duplicated(subset=available, keep=False)]
                n_dupes = len(dupes) // 2  # pairs
                if n_dupes > 0:
                    issues.append({
                        "severity": "warning" if n_dupes < 5 else "error",
                        "title": f"{n_dupes} possible duplicate transaction(s)",
                        "detail": f"Found {len(dupes)} rows with identical Date+Type+Title+Net. Could be duplicated CSV imports.",
                        "metric": "duplicates",
                    })
    except Exception:
        pass

    # ── 8. Sales tax ratio — tax collected should be 0–15% of gross ─────────
    try:
        if gross_sales > 0 and total_taxes > 0:
            tax_pct = (total_taxes / gross_sales) * 100
            if tax_pct > 15:
                issues.append({
                    "severity": "error",
                    "title": "Sales tax ratio too high",
                    "detail": f"Tax collected is {tax_pct:.1f}% of gross sales (expected 0–15%). Tax rows may be duplicated or miscategorized.",
                    "metric": "tax_ratio",
                })
    except Exception:
        pass

    # ── 9. Negative stock — items used more than purchased ──────────────────
    try:
        if len(STOCK_SUMMARY) > 0:
            neg = STOCK_SUMMARY[STOCK_SUMMARY["in_stock"] < 0]
            if len(neg) > 0:
                names = ", ".join(neg["display_name"].head(5).tolist())
                extra = f" (+{len(neg)-5} more)" if len(neg) > 5 else ""
                issues.append({
                    "severity": "warning",
                    "title": f"{len(neg)} item(s) with negative stock",
                    "detail": f"More used than purchased: {names}{extra}. Check usage log or receipt quantities.",
                    "metric": "negative_stock",
                })
    except Exception:
        pass

    # ── 10. Order value outliers — orders > 3 std devs from mean ────────────
    try:
        if len(sales_df) > 10:
            vals = sales_df["Net_Clean"]
            mean_v = vals.mean()
            std_v = vals.std()
            if std_v > 0:
                outliers = sales_df[abs(vals - mean_v) > 3 * std_v]
                if len(outliers) > 0:
                    high = outliers["Net_Clean"].max()
                    issues.append({
                        "severity": "info",
                        "title": f"{len(outliers)} order value outlier(s)",
                        "detail": f"Orders more than 3 std devs from mean ({money(mean_v)}). Largest: {money(high)}. Likely legitimate but worth verifying.",
                        "metric": "outliers",
                    })
    except Exception:
        pass

    # ── 11. Negative values — revenue, order count, inv cost should be >= 0 ─
    try:
        negatives = []
        if gross_sales < 0:
            negatives.append(f"gross_sales={money(gross_sales)}")
        if order_count < 0:
            negatives.append(f"order_count={order_count}")
        if total_inventory_cost < 0:
            negatives.append(f"inventory_cost={money(total_inventory_cost)}")
        if negatives:
            issues.append({
                "severity": "error",
                "title": "Negative metric values",
                "detail": f"These should never be negative: {', '.join(negatives)}. Data may be corrupted or mislabeled.",
                "metric": "negative_values",
            })
    except Exception:
        pass

    # ── 12. Monthly trend breaks — >50% month-over-month swing ──────────────
    try:
        if len(monthly_sales) >= 2:
            sorted_months = sorted(monthly_sales.index)
            for i in range(1, len(sorted_months)):
                prev_m = sorted_months[i - 1]
                curr_m = sorted_months[i]
                prev_v = monthly_sales[prev_m]
                curr_v = monthly_sales[curr_m]
                if prev_v > 0:
                    change_pct = ((curr_v - prev_v) / prev_v) * 100
                    if abs(change_pct) > 50:
                        direction = "increase" if change_pct > 0 else "decrease"
                        issues.append({
                            "severity": "info",
                            "title": f"Large {direction} in {curr_m}",
                            "detail": f"Sales went from {money(prev_v)} to {money(curr_v)} ({change_pct:+.0f}%). Could indicate missing data or seasonal shift.",
                            "metric": "trend_break",
                        })
    except Exception:
        pass

    # ── 13. Receipt-to-bank gap — large unmatched amounts ───────────────────
    try:
        if bank_total_debits > 0 and total_inventory_cost > 0:
            bank_inv = bank_by_cat.get("Amazon Inventory", 0)
            gap = abs(total_inventory_cost - bank_inv)
            if gap > 200 and bank_inv > 0:
                issues.append({
                    "severity": "info",
                    "title": "Receipt-to-bank inventory gap",
                    "detail": f"Receipt total ({money(total_inventory_cost)}) vs bank 'Amazon Inventory' ({money(bank_inv)}). Gap: {money(gap)}. Some purchases may use other payment methods.",
                    "metric": "receipt_bank_gap",
                })
    except Exception:
        pass

    # ── 14. Fee-to-sale pairing — every Sale should have Transaction fee ────
    try:
        if len(sales_df) > 0 and len(fee_df) > 0:
            sale_orders = set(
                sales_df["Title"].str.extract(r"(Order #\d+)", expand=False).dropna()
            )
            fee_orders = set(
                fee_df[fee_df["Title"].str.startswith("Transaction fee:", na=False)]
                ["Info"].dropna()
            )
            sales_no_fee = sale_orders - fee_orders
            if len(sales_no_fee) > 0:
                sample = ", ".join(list(sales_no_fee)[:3])
                extra = f" (+{len(sales_no_fee)-3} more)" if len(sales_no_fee) > 3 else ""
                issues.append({
                    "severity": "warning",
                    "title": f"{len(sales_no_fee)} sale(s) missing transaction fees",
                    "detail": f"Orders with no matching fee row: {sample}{extra}. Could be fee credits or data gaps.",
                    "metric": "fee_pairing",
                })
    except Exception:
        pass
...
if AUDIT_RESULTS:
    _sev_counts = {}
    for _r in AUDIT_RESULTS:
        _sev_counts[_r["severity"]] = _sev_counts.get(_r["severity"], 0) + 1
    _sev_str = ", ".join(f"{v} {k}s" for k, v in _sev_counts.items())
    print(f"  Data Audit: {len(AUDIT_RESULTS)} issues found ({_sev_str})")
