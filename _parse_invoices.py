...
def parse_personal_order(text, fname):
    ...
    # Extract items - look for product descriptions followed by prices
    items = []
    # Find price lines at end of each product block
    product_blocks = re.findall(
        r"(?:Delivered|Return window).+?\n(.+?)\nSold by:\s*(.+?)\n.+?\n\$([\d,]+\.\d+)",
        text, re.DOTALL,
    )
    if not product_blocks:
        # Simpler: just find "PRODUCT_NAME\nSold by: ...\n...\n$PRICE"
        product_blocks2 = re.findall(
            r"\n([A-Z].+?)\nSold by:\s*(.+?)\n.*?\$([\d,]+\.\d+)", text, re.DOTALL
        )
        for name_raw, seller, price in product_blocks2:
            name = " ".join(name_raw.split())[:200]
            items.append({"qty": 1, "name": name, "price": float(price.replace(",", "")),
                          "seller": seller.strip(), "ship_to": ship_address})
    else:
        for name_raw, seller, price in product_blocks:
            name = " ".join(name_raw.split())[:200]
            items.append({"qty": 1, "name": name, "price": float(price.replace(",", "")),
                          "seller": seller.strip(), "ship_to": ship_address})
...
paper_receipts = [
    ...
    },
    {
        ...
        "tax": 1.32,
        "items": [
            {"qty": 1, "name": "12 IN. X 25 FT. Orange Heavy Duty Tape", "price": 9.98,
             "seller": "Home Depot", "ship_to": "4041 S Sheridan, Tulsa, OK 74145"},
            {"qty": 1, "name": "HD Small Box 16x10x12", "price": 5.56,
             "seller": "Home Depot", "ship_to": "4041 S Sheridan, Tulsa, OK 74145"},
        ],
        "source": "Home Depot",
        "file": "Paper Receipts.pdf (page 2)",
        "ship_address": "4041 S Sheridan, Tulsa, OK 74145",
        "payment_method": "Unknown (paper receipt)",
    },
    {
        ...
        "items": [
            {"qty": 1, "name": "Crafts supplies", "price": 5.58,
             "seller": "Hobby Lobby", "ship_to": "3132 E 51st St, Tulsa, OK 74105"},
            {"qty": 1, "name": "Crafts supplies", "price": 1.41,
             "seller": "Hobby Lobby", "ship_to": "3132 E 51st St, Tulsa, OK 74105"},
        ],
        "source": "Hobby Lobby",
        "file": "Paper Receipts.pdf (page 3)",
        "ship_address": "3132 E 51st St, Tulsa, OK 74105",
        "payment_method": "Unknown (paper receipt)",
    },
    {
    ...
    },
    {
        ...
        "tax": 2.85,
        "items": [
            {"qty": 1, "name": "Luminate 3D Printer Magnetic Build Plate 256x256x2, X1C/X1/P1P/P1S Compatible G10 Garolite FR4", "price": 38.50,
             "seller": "Luminate 3D", "ship_to": "Thomas, 2725 E 47TH ST, TULSA, OK 74105-5346"},
            {"qty": 1, "name": "WOKSEDO 12 X 3g Super Glue Liquid, Clear Strong Adhesive, Fast Drying Cyanoacrylate Glue", "price": 4.99,
             "seller": "Wueedo", "ship_to": "Thomas, 2725 E 47TH ST, TULSA, OK 74105-5346"},
        ],
        "source": "Personal Amazon",
