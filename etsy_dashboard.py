    ...
    for _idx, _o in enumerate(_ledger_orders):
        ...
        _detail_section = html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div("ORDER INFO", style={"color": CYAN, "fontSize": "10px", "fontWeight": "bold", "marginBottom": "6px", "letterSpacing": "1px"}),
                    html.Div([html.Span("Order #", style=_detail_label), html.Span(f"{_oid}", style={**_detail_val, "color": CYAN})], style=_detail_row_style),
                    html.Div([html.Span("Date", style=_detail_label), html.Span(f"{_date}", style=_detail_val)], style=_detail_row_style),
                    html.Div([html.Span("Buyer", style=_detail_label), html.Span(f"{_buyer}", style=_detail_val)], style=_detail_row_style),
                    html.Div([html.Span("Status", style=_detail_label), html.Span(f"{_status}", style={**_detail_val, "color": _status_color})], style=_detail_row_style),
                    html.Div([html.Span("Store", style=_detail_label), html.Span(f"{_store_names.get(_store_slug, _store_slug)}", style=_detail_val)], style=_detail_row_style),
                    ...
                    html.Div([html.Span("Sale Price (items)", style=_detail_label), html.Span(f"${_sale_price:.2f}", style=_detail_val)], style=_detail_row_style),
                    html.Div([html.Span("Buyer Shipping", style=_detail_label), html.Span(f"${_buyer_ship:.2f}", style={**_detail_val, "color": GREEN if _buyer_ship > 0 else GRAY})], style=_detail_row_style),
                    ...
                    html.Div([html.Span("Total Buyer Paid", style={**_detail_label, "fontWeight": "bold"}), html.Span(f"${_sale_price + _buyer_ship + (_o.get('Sales Tax', 0) or 0):.2f}", style={**_detail_val, "fontWeight": "bold", "color": GREEN})], style=_detail_row_style),
                        ...
                        html.Button("Save", id={"type": "shipper-assign-btn", "order": _oid}, n_clicks=0,
                            style={"fontSize": "10px", "padding": "3px 8px", "backgroundColor": f"{BLUE}25",
                                   "border": f"1px solid {BLUE}", "borderRadius": "4px", "color": BLUE, "cursor": "pointer", "marginLeft": "6px"}),
                                        html.Span(id={"type": "shipper-assign-status", "order": _oid}, style={"fontSize": "11px", "marginLeft": "6px"}),
...
@app.callback(
    Output({"type": "shipper-assign-status", "order": MATCH}, "children"),
    Output({"type": "shipper-assign-btn", "order": MATCH}, "children"),
    Output({"type": "shipper-assign-btn", "order": MATCH}, "style"),
    Input({"type": "shipper-assign-btn", "order": MATCH}, "n_clicks"),
    State({"type": "shipper-assign-dd", "order": MATCH}, "value"),
    prevent_initial_call=True,
)
def assign_shipper_to_order(n_clicks, shipper_value):
    ...
    order_id = trigger_id.get("order", "") if isinstance(trigger_id, dict) else ""

    if not order_id:
        raise dash.exceptions.PreventUpdate

    shipper = str(shipper_value or "").strip()
    if shipper not in ("TJ", "Braden"):
        return (
            html.Span("Pick TJ or Braden first", style={"color": ORANGE}),
            "Save",
            {"fontSize": "10px", "padding": "3px 8px", "backgroundColor": f"{BLUE}25",
             "border": f"1px solid {BLUE}", "borderRadius": "4px", "color": BLUE, "cursor": "pointer", "marginLeft": "6px"},
        )
    ...
    try:
        ...
        for o in orders:
            if str(o.get("Order ID")) == str(order_id):
                ...
                _color = CYAN if shipper == "TJ" else GREEN
                return (
                    html.Span(f" {shipper}", style={"color": _color, "fontWeight": "bold"}),
                    "Saved",
                    {"fontSize": "10px", "padding": "3px 8px", "backgroundColor": f"{GREEN}25",
                     "border": f"1px solid {GREEN}", "borderRadius": "4px", "color": GREEN, "marginLeft": "6px"},
                )
        return (
            html.Span("Order not found", style={"color": RED}),
            "Save",
            {"fontSize": "10px", "padding": "3px 8px", "backgroundColor": f"{BLUE}25",
             "border": f"1px solid {BLUE}", "borderRadius": "4px", "color": BLUE, "cursor": "pointer", "marginLeft": "6px"},
        )
        ...
        return (
            html.Span(f"Error: {e}", style={"color": RED, "fontSize": "10px"}),
            "Save",
            {"fontSize": "10px", "padding": "3px 8px", "backgroundColor": f"{BLUE}25",
             "border": f"1px solid {BLUE}", "borderRadius": "4px", "color": BLUE, "cursor": "pointer", "marginLeft": "6px"},
        )
...
@app.callback(
    ...
    Input({"type": "current-inv-save", "idx": ALL}, "n_clicks"),
    State({"type": "current-inv-qty-input", "idx": ALL}, "value"),
    prevent_initial_call=True,
)
def save_current_inv_qty(all_clicks, all_qtys):
    """Save a single item's quantity update."""
    ctx = callback_context
    if not ctx.triggered:
        raise dash.exceptions.PreventUpdate
    trigger = ctx.triggered[0]
...
@app.callback(
    ...
    Output("current-inv-status", "children", allow_duplicate=True),
...
def add_current_inv_item(add_clicks, name, qty, location, category):
    """Add a new item to current inventory."""
    if not add_clicks:
        raise dash.exceptions.PreventUpdate
    ...
    if not name or not name.strip():
        return dash.no_update, html.Span("Enter an item name", style={"color": ORANGE, "fontSize": "12px"}), dash.no_update, dash.no_update
    ...
    return _build_current_inv_table(), html.Span(f"Added {name.strip()}", style={"color": GREEN, "fontSize": "12px"}), "", ""
